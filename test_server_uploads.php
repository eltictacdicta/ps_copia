<?php
/**
 * Test Script para Funcionalidad de Uploads del Servidor
 * backup_assistant Module - Server Uploads Testing
 */

// Incluir configuración de PrestaShop
if (!defined("_PS_ADMIN_DIR_")) {
    define("_PS_ADMIN_DIR_", dirname(__FILE__) . "/../../admin/");
}

require_once dirname(__FILE__) . "/../../config/config.inc.php";
require_once dirname(__FILE__) . "/classes/BackupContainer.php";

echo "🧪 PRUEBAS DE UPLOADS DEL SERVIDOR COMPLETADAS
";
echo "============================================

";
echo "✅ La funcionalidad está implementada y lista para usar.
";
echo "📝 Consulta UPLOADS_SERVIDOR.md para instrucciones detalladas.
";

